...
void EctoPID::update(){
    ...
    if(std::abs(errorIntegral) > config.maxAbsoluteItegral && config.maxAbsoluteItegral != -1){
        errorIntegral = std::copysign(config.maxAbsoluteItegral, errorIntegral);
    }
...
void EctoPID::setConfig(const PIDConfig &config) {
    ...
    if(config.maxAbsoluteItegral != -1 && config.maxAbsoluteItegral < 0){
        throw std::invalid_argument("EctoPID: config.maxAbsoluteItegral is less than zero, should be an absolute value");
    }
