...
EctoSMC::EctoSMC(EctoControllerSource &source, EctoControllerOutput &output, const EctoSMCConfig &config) : source(source), output(output) {
    setConfig(config);
}
...
void EctoSMC::update() {
    ...
    output.outputSet(ka * std::sqrt(std::abs(slidingSurfaceResult)) * sign(slidingSurfaceResult) + config.k2 * slidingSurfaceResult);
}
