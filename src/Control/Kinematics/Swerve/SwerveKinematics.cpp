...
SwerveMotorValues SwerveKinematics::OLDcalculateInverseKinematics(const Twist2D &targetVelocity,
                                                                  const SwerveMotorValues &currentState) {
    double dt = std::chrono::duration_cast<std::chrono::duration<double>>(std::chrono::high_resolution_clock::now() - lastRunTime).count();
...
//    output.normalizeVelocities();
    ...
    output = SwerveMotorValues::optimizeValues(currentState, output, lastState, lastSet, dt);
    lastSet = output;
...
SwerveMotorValues SwerveKinematics::calculateInverseKinematics(const Twist2D &targetVelocity, const SwerveMotorValues& currentState, const Point2D &centerOfRotation) {
    double dt = std::chrono::duration_cast<std::chrono::duration<double>>(std::chrono::high_resolution_clock::now() - lastRunTime).count();
...
SwerveMotorValues SwerveKinematics::calculateInverseKinematics(const Twist2D &targetVelocity,
                                                               const SwerveMotorValues &currentState) {
...
Twist2D SwerveKinematics::calculateForwardKinematics(const SwerveMotorValues &currentValues) const {
    Eigen::Matrix<double, 8, 1> moduleStatesMatrix;
    moduleStatesMatrix <<
        //Top left
