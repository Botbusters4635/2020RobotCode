...
public:
    ...
    SwerveMotorValues calculateInverseKinematics(const Twist2D &targetVelocity, const SwerveMotorValues& currentState, const Point2D &centerOfRotation);
    SwerveMotorValues calculateInverseKinematics(const Twist2D &targetVelocity, const SwerveMotorValues& currentState);

    SwerveMotorValues OLDcalculateInverseKinematics(const Twist2D &targetVelocity, const SwerveMotorValues& currentState);

    //Returns delta in x, y, theta
    Twist2D calculateForwardKinematics(const SwerveMotorValues &currentValues) const;
...
private:
    ...
    SwerveMotorValues lastState, lastSet;
