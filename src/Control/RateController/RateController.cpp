...
RateController::RateController(RateControllerConfig config) {
    if(config.maxRate <= 0.0){
        throw std::invalid_argument("Max rate cannot be less than or equal zero, given value: " + std::to_string(config.maxRate));
    }
    ...
    if(config.maxValue != 0 or config.minValue != 0){
        if(config.minValue >= config.maxValue){
            throw std::invalid_argument("Min value is greater than max value, given values: Min " + std::to_string(config.minValue) + " Max " + std::to_string(config.maxValue));
        }
