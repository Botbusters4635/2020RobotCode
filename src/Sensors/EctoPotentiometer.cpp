...
EctoPotentiometer::EctoPotentiometer(int port, double potentiometerRange, double lowerVoltageLimit, double upperVoltageLimit, double voltageOffset) {
...
double EctoPotentiometer::PIDGet() {
	return getPosition();
}
...
double EctoPotentiometer::getPosition() const {
	lastRecordedPosition = (input->GetAverageVoltage() * potentiometerRange / (upperVoltageLimit - lowerVoltageLimit)) - voltageOffset;
	lastPositionTime = frc::Timer::GetFPGATimestamp();
...
double EctoPotentiometer::getVelocity() const {
	return (getPosition() - lastRecordedPosition) / (frc::Timer::GetFPGATimestamp() - lastPositionTime);
}
